...
use std::fs;
use gtk4::{FileChooserDialog, FileChooserAction, ResponseType, TreeStore, ScrolledWindow, TextView};
use gtk4::prelude::*;
use std::rc::Rc;
        ...
        if response == ResponseType::Accept {
            if let Some(file) = dialog.file() {
                if let Some(path) = file.path() {
                    crate::actions::open_file_in_new_tab(
                        &path,
                        &notebook,
                        &highlight_closure,
                        &buffer_paths,
        ...
        if response == ResponseType::Accept {
            if let Some(folder) = dialog.file() {
                if let Some(path) = folder.path() {
                    crate::actions::open_directory_in_tree(
                        &path,
                        &tree_store,
                        &app_settings,
                    );
...
pub fn update_tab_label(notebook: &gtk4::Notebook, buffer: &gtk4::TextBuffer, path: &std::path::Path) {
                ...
                if let Some(path) = file.path() {
                    ...
                    match std::fs::write(&path, content) {
                        Ok(_) => {
                            ...
                            buffer_paths_clone.borrow_mut().insert(buffer_clone.clone(), path.clone());
...
pub fn populate_tree_view(tree_store: &TreeStore, path: &std::path::Path) {
    ...
    if path.parent().is_some() {
        ...
        tree_store.insert_with_values(None, None, &[(0, &".."), (1, &parent_path.to_str().unwrap_or(""))]);
    }
    ...
    if let Ok(entries) = fs::read_dir(path) {
        ...
        for entry_path in directories {
            let file_name = entry_path.file_name().and_then(|s| s.to_str()).unwrap_or("").to_string();
